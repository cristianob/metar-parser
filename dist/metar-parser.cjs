...
// src/metar-parser.ts
var metar_parser_exports = {};
__export(metar_parser_exports, {
  default: () => metar_parser_default
});
module.exports = __toCommonJS(metar_parser_exports);
var REPORT_TYPES = /* @__PURE__ */ new Set(["METAR", "SPECI"]);
var CLOUDS = {
...
var WEATHER_DESCRIPTOR_CODES = /* @__PURE__ */ new Set(["MI", "PR", "BC", "DR", "BL", "SH", "TS", "FZ"]);
var WEATHER_PHENOMENA_CODES = /* @__PURE__ */ new Set([
  "DZ",
  "RA",
  "SN",
  "SG",
...
var MAX_WEATHER_KEY_LENGTH = Object.keys(WEATHER).reduce((max, key) => Math.max(max, key.length), 0);
function createError(message, tokenIndex, token) {
...
function parseAbbreviation(source, map) {
...
function parseWeatherAbbreviations(token, result = []) {
...
function buildReportTime(day, hour, minute) {
...
function parseDateTimeGroup(token) {
...
function parseCorrectionToken(token) {
...
function parseWindSpeedPart(raw) {
  if (!raw) return null;
  return {
    raw,
...
function parseVisibilityGroup(token, nextToken) {
...
function parseDirectionalVisibility(token) {
...
function isTrendStartToken(token) {
  return token === "NOSIG" || token === "BECMG" || token === "TEMPO";
}
function parseTrendTimeToken(token) {
...
function parseRVR(token) {
  ...
  return {
    ...
    separator: match[3] || "/",
    minIndicator: match[4] || null,
    minValue: match[5] || null,
    variableIndicator: match[6] || null,
    maxIndicator: match[7] || null,
    maxValue: match[8] || null,
    trend: match[9] || null,
    unitsOfMeasure: match[10] || null,
    unit: match[10] || "m"
  };
}
function parseRunwayState(token) {
  if (!token) return null;
...
function parseCloud(token) {
  ...
  return {
    ...
    convectiveType: match[3] && match[3] !== "///" ? match[3] : null,
    convectiveTypeUnavailable: match[3] === "///" ? true : void 0
  };
}
function parseTemperatureDewpoint(token) {
...
function parseSignedTemperature(part) {
...
function parseRecentWeather(token) {
...
function parseWindShear(tokens, index) {
...
function isTrendBoundary(token) {
...
function parseTrendSection(tokens, startIndex) {
